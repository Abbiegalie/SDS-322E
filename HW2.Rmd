...
*The object letters is a character data type of – found through using the type of command – which is a built in object of the alphabet which contains 26 elements – found through using the length command. *
...
*The number of False elements in the logical vector is 23.*
...
```{R}
letters[c(5, 9, 13, 14, 15, 19)]
```

*The letters to the name appear backwards in order.*
...
get_pos <- function(name2) which(letters %in% name2)
...
```{R}
name <- c("c", "o", "n", "f", "i", "d", "e", "n", "t", 
    "i", "a", "l")

result <- sapply(name, get_pos)

if (result + 12 < 27) {
    letters[result + 12]
} else {
    letters[result + 12%%26]
}
```
